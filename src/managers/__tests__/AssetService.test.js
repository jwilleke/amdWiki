 ...
 * and graceful degradation when managers are unavailable.
 */
...
describe('AssetService.search()', () => {
  ...
  describe('max cap', () => {
    it('respects max option', async () => {
      ...
      const { service } = makeService({ attachments: manyAttachments, noMedia: true });

      const results = await service.search({ types: ['attachment'], max: 5 });
  ...
  describe('graceful degradation', () => {
    ...
    it('both managers unavailable → returns empty array', async () => {
