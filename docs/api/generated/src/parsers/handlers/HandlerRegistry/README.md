[**ngdpbase API v1.5.0**](../../../../README.md)

***

[ngdpbase API](../../../../README.md) / src/parsers/handlers/HandlerRegistry

# src/parsers/handlers/HandlerRegistry

## Classes

- [HandlerRegistrationError](classes/HandlerRegistrationError.md)
- [HandlerRegistry](classes/HandlerRegistry.md)

## Interfaces

- [DependencyValidationError](interfaces/DependencyValidationError.md)
- [ExportedRegistryState](interfaces/ExportedRegistryState.md)
- [ExtendedRegistryStats](interfaces/ExtendedRegistryStats.md)
- [HandlerInitContext](interfaces/HandlerInitContext.md)
- [RegistrationOptions](interfaces/RegistrationOptions.md)
- [RegistryConfig](interfaces/RegistryConfig.md)
- [RegistryInfo](interfaces/RegistryInfo.md)
- [RegistryStats](interfaces/RegistryStats.md)
- [WikiEngine](interfaces/WikiEngine.md)

## References

### default

Renames and re-exports [HandlerRegistry](classes/HandlerRegistry.md)

***

### DependencySpec

Re-exports [DependencySpec](../BaseSyntaxHandler/interfaces/DependencySpec.md)
