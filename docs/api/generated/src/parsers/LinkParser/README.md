[**ngdpbase API v1.5.0**](../../../README.md)

***

[ngdpbase API](../../../README.md) / src/parsers/LinkParser

# src/parsers/LinkParser

## Classes

- [Link](classes/Link.md)
- [LinkParser](classes/LinkParser.md)

## Interfaces

- [DefaultClasses](interfaces/DefaultClasses.md)
- [InterWikiSiteConfig](interfaces/InterWikiSiteConfig.md)
- [LinkAttributes](interfaces/LinkAttributes.md)
- [LinkData](interfaces/LinkData.md)
- [LinkInfo](interfaces/LinkInfo.md)
- [LinkParserOptions](interfaces/LinkParserOptions.md)
- [ParserContext](interfaces/ParserContext.md)
- [ParserStats](interfaces/ParserStats.md)
- [SecurityOptions](interfaces/SecurityOptions.md)
- [UrlPatterns](interfaces/UrlPatterns.md)

## Type Aliases

- [LinkType](type-aliases/LinkType.md)

## Variables

- [default](variables/default.md)
