...
The server checks for a `.install-complete` marker file at `${FAST_STORAGE}/.install-complete` on every request. If the file does not exist, the setup wizard is shown instead of the wiki.
...
| Base URL | Yes | Full URL users use to reach this instance (e.g. `http://wiki.example.com`) — used in links, RSS, and Schema.org metadata |
...
A checkbox lets you copy all `required-pages/` content into your wiki's pages directory. This is checked by default and recommended — it populates the wiki with documentation, help pages, and system pages needed for full functionality.
