...
To embed a single media library photo inline in page content — rather than a full album — use the `media://` URI with [ImagePlugin] or [AttachPlugin]:
...
For layout options (`align`, `display`, `caption`, `width`, etc.) see [Attachments], [ImagePlugin], or [AttachPlugin].
...
- [Media] — overview and quick-start for the media feature
- [Attachments] — uploading files and choosing between Image / ATTACH plugins
- [ImagePlugin] — `[{Image}]` parameter reference
- [AttachPlugin] — `[{ATTACH}]` parameter reference
- [MediaPlugin] — `[{MediaPlugin}]` parameter reference
