...
| Key | Default | Description |
| --- | ------- | ----------- |
| `ngdpbase.translator-reader.match-english-plurals` | `true` | Allow wiki links to match plural/singular page name variants |
| `ngdpbase.translator-reader.camel-case-links` | `true` | Automatically link CamelCase words as wiki page names |
| `ngdpbase.translator-reader.allow-html` | `false` | Allow raw HTML in wiki markup |
...
These settings control the initial installation wizard. Completion is tracked via `$FAST_STORAGE/.install-complete` — not via config.

| Key | Default | Description |
| --- | ------- | ----------- |
| `ngdpbase.install.require-setup` | `true` | Show the setup wizard if `.install-complete` does not exist |
| `ngdpbase.install.copy-startup-pages` | `true` | Copy `required-pages/` into the page store on first run |
| `ngdpbase.install.create-admin-user` | `true` | Create the default admin user on first run |
| `ngdpbase.install.organization.name` | `""` | Short organization name used in page headers and metadata |
| `ngdpbase.install.organization.legal-name` | `""` | Legal organization name for Schema.org output |
| `ngdpbase.install.organization.description` | `""` | Short description of the organization |
| `ngdpbase.install.organization.founding-date` | `""` | ISO 8601 date the organization was founded |
| `ngdpbase.install.organization.contact-email` | `""` | Public contact email address |
| `ngdpbase.install.organization.address-locality` | `""` | City |
| `ngdpbase.install.organization.address-region` | `""` | State or region |
| `ngdpbase.install.organization.address-country` | `""` | Two-letter ISO country code |
...
| `ngdpbase.search.provider.elasticsearch.indexname` | `"ngdpbase"` | Index name in Elasticsearch |
...
| ------- | ---------------- | ------- |
| `plugin` | `90` | Wiki plugin tags `[{PluginName ...}]` |
| `wikitag` | `95` | Wiki link tags `[[PageName]]` |
...
| `ngdpbase.markup.filters.validation.validate-markup` | `true` | Validate wiki markup syntax |
...
| `ngdpbase.features.maintenance.allow-admins` | `true` | Allow admins to access the wiki during maintenance mode |
