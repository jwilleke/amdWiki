...
lastModified: '2025-10-17T12:58:27.310Z'
editor: system
...
**Output:** Port: [{ConfigAccessor key='ngdpbase.server.port' valueonly='true'}]
...
### Display Enabled User Keywords Only
...
### Display Only Required-Pages Categories
...
| `type` | `roles`, `permissions`, `policy-summary`, `user-summary`, `actions`, `manager`, `feature`, `userKeywords`, `systemKeywords`, `systemCategories` | *(none)* | Yes (if no `key`) | What type of configuration to display |
...
| `enabled` | `true`, `false` | *(none)* | No | Filter keywords/categories by enabled status (use with `userKeywords`, `systemKeywords`, `systemCategories`) |
| `label` | Label string | *(none)* | No | Filter by specific label (use with `userKeywords`, `systemKeywords`, `systemCategories`) |
| `category` | Category string | *(none)* | No | Filter by category (use with `userKeywords`, `systemKeywords`) |
| `default` | `true`, `false` | *(none)* | No | Filter by default flag (use with `systemKeywords`, `systemCategories`) |
| `storageLocation` | `regular`, `required` | *(none)* | No | Filter by storage location (use with `systemCategories`) |
...
```wiki
The application is running version [{ConfigAccessor key='ngdpbase.version' valueonly='true'}].
```

**Output:** The application is running version [{ConfigAccessor key='ngdpbase.version' valueonly='true'}].
...
```wiki
[{ConfigAccessor key='ngdpbase.application-name'}]
```
...
```wiki
Server port is [{ConfigAccessor key='ngdpbase.server.port' valueonly='true'}].
```

Output: Server port is [{ConfigAccessor key='ngdpbase.server.port' valueonly='true'}].
...
```wiki
Roles: [{ConfigAccessor key='ngdpbase.roles.definitions.*.name' valueonly='true' after=', '}]
```
...
```wiki
Welcome to [{ConfigAccessor key='ngdpbase.application-name' valueonly='true' after=''}]!
Running version [{ConfigAccessor key='ngdpbase.version' valueonly='true' after=''}].
```
...
**Port:** [{ConfigAccessor key='ngdpbase.server.port' valueonly='true' after=''}]
**Host:** [{ConfigAccessor key='ngdpbase.server.host' valueonly='true' after=''}]
...
[{ConfigAccessor key='ngdpbase.features.*' valueonly='true' before='* ' after='\n'}]
```
...
- [Platform Variables] - For dynamic wiki variables
