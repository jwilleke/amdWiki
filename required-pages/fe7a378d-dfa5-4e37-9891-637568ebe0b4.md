...
- **Instant Results**: Lunr.js provides fast client-side search (default for smaller instances)
- **Indexed Content**: All pages pre-indexed for maximum speed
- **Smart Filtering**: Efficient category and keyword filtering
- **Elasticsearch option**: For sites with 10,000+ pages, administrators can switch to the `ElasticsearchSearchProvider` for persistent indexing and scalable multi-node deployments — see your administrator for details
