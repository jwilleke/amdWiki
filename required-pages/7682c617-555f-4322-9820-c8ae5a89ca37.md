...
[BROKEN.pages] **<<-- Needs rewritten to reflect reality**

Understanding where and how wiki pages are stored in amdWiki.

## System Initialization

On a new system the system Initialization runs entries within `required-pages/` are copied to `pages/`.

On a exisiting system the system Initialization determines there are exising entries within `pages/` a choice can be made tp copy `required-pages/` to `pages/`.

Pages are stored by [Page Metadata Documentation] and specifically [system-category] as a top-level category that determines the Audiance.

## Quick Overview

amdWiki keeps all pages within `pages/`.
...
| Category | Description | Example | Where Stored |
| ---------- | ------------- | --------- |
