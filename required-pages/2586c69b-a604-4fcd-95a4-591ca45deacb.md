...
Private pages are wiki pages that are visible only to their **creator** and to **administrators**. All other users — including other authenticated users — are denied access and receive an *Access Denied* response when they attempt to view, edit, or navigate to the page.
...
The page file is moved back to the standard pages area and immediately becomes visible to all users who have access to the wiki.
...
Pages in the `documentation`, `system`, and other built-in categories (stored in `required-pages/`) are committed to the project and are always publicly visible within the wiki. Attempting to add the `private` keyword to a required page returns an error:

> Required pages cannot be marked as private.

### Sharing with specific users

The current implementation is all-or-nothing: a private page is visible only to its creator and admins. There is no mechanism to share a private page with a specific non-admin user. If you need to share content with a subset of users, consider the standard wiki with role-based access, or publish the page and rely on wiki-level authentication.
...
- **Not for secrets.** Private pages are hidden from other wiki users, but not encrypted on disk. Do not store passwords, API keys, or other sensitive credentials.
